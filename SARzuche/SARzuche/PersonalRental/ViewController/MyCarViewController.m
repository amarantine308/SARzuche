...
-(void)updateOrderData
{
    ...
    NSString *backTime = [NSString stringWithFormat:@"%@", [[PublicFunction ShareInstance] getYMDHMString:m_myOrderData.m_returnTime]];
...
- (void)sendMyLocation
{
    ...
    [[LoadingClass shared] showLoadingForMoreRequest:STR_PLEASE_WAIT];
    NSString *latitude = [NSString stringWithFormat:@"%.6f",myLocation.latitude];
    NSString *longitude = [NSString stringWithFormat:@"%.6f",myLocation.longitude];
...
//    [self showWaiting];
    ...
    if (NO == [[PublicFunction ShareInstance] IsTimeToTake:curDate])
    {
...
//    [self showWaiting];
    ...
    if (NO == [[PublicFunction ShareInstance] IsTimeToTake:curDate])
    {
        [self showAlertView:@"订单开始前5分钟可使用车辆"];
        return;
    }
    [self sendMyLocation];
  /*  NSString *orderId = [NSString stringWithFormat:@"%@", GET(m_myOrderData.m_orderNum)];
